"""Match every settled Polymarket NFL and college football moneyline to ESPN's final score.

Input: football-games.csv (the read-only export of settled football moneylines; columns
condition_id, event_slug, league, away, home, away_token, home_token, game_start_utc, winning_outcome, volume_usd).
For each game date (US Eastern, plus or minus a day) it reads ESPN's public scoreboard
(site.api.espn.com/apis/site/v2/sports/football/{nfl|college-football}/scoreboard, college football for the FBS
and FCS groups), matches the game whose two teams carry the Polymarket names and whose listed start is nearest
the market's, and records:
...
Usage: python3 football-scoreboard.py football-games.csv football-scoreboard.csv
"""
...
URLS = {
    "nfl": ["https://site.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard?dates={date}&limit=300"],
    "cfb": [
        "https://site.api.espn.com/apis/site/v2/sports/football/college-football/scoreboard?dates={date}&groups=80&limit=300",
        "https://site.api.espn.com/apis/site/v2/sports/football/college-football/scoreboard?dates={date}&groups=81&limit=300",
    ],
}
...
def main(games_path, output_path):
    ...
    for game in games:
        ...
        # How many completed games between the same two teams the three-day window holds.
